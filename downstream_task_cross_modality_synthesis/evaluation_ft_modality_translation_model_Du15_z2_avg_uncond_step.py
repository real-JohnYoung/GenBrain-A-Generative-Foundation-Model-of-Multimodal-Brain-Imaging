...
def main(args):
    ...
    transform = True
    dataset = UKBDataset_modality_translation_DU15_zstat_evaluation_uncon(args.data_path, args.sample_file,  image_data_info ,transform=transform)
    loader = DataLoader(dataset,batch_size=batch_size,shuffle=False,num_workers=args.num_workers)

    for i,batch_data in enumerate(loader):  # (0, n, batch_size)

        for sample_num in range(average_num):

            eid_batch = batch_data['eid']
            modality_batch = batch_data['mod']
            ...
            y3 = modality_batch.to(device)       # modality 

            source_x =  batch_data['DU15_image'].to(device)               # attention
            ...
            save_batch_data(eid_batch.numpy(), modality_batch.numpy(),samples.cpu(), save_dir, image_data_info,sample_num)

            print(f"Batch {i + 1} saved successfully, sample num {sample_num}, saved successfully.")

        if args.need_average:
            get_average_samples(eid_batch.numpy(),modality_batch.numpy(),average_num, save_dir)
...
def get_average_samples(eid_batch, modality_batch, average_num, save_dir):
    ...
    for i in range(sub_num):
        ...
        if modality == 16:
            modality_name = 'zstat1s'
        elif modality == 17:
            modality_name = 'zstat2s'
        elif modality == 18:
            modality_name = 'zstat5s'
...
def save_batch_data(eid_batch,modality_batch, data, save_dir, image_data_info, sample_num):
    sub_num = len(eid_batch)
    for i in range(sub_num):
        addr = os.path.join(save_dir, str(eid_batch[i]))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str)# ukb_multimodal
    ...
    parser.add_argument("--ckpt", type=str)#000-DiT-UKB-L-256-finetune_mode-full/checkpoints/step_0050000.pt
