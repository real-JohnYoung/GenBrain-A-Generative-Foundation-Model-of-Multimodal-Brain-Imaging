...
from model_utils import UKBDataset_modality_translation_DU15_zstat_finetune_uncon,PatchEmbed_1d
...
class DiT_UKB_finetune(nn.Module):
    ...
    def __init__(self, dit_model, finetune_mode='full', finetune_block=None):
        ...
        self.dit_model.x_embedder = PatchEmbed_1d(input_size=self.dit_model.x_embedder.input_size, 
                                  ...
                                  in_chans=16, embed_dim=self.hidden_size)
    ...
    def forward(self, x, t, y1, y2, y3, source_x):
        ...
        y1 = self.dit_model.y1_embedder(y1, self.training, drop_mask)  # (N, D)  source_age
...
def main(args):
    ...
    if "ema" in checkpoint:
        ...
        if 'x_embedder.proj.weight' in checkpoint_ema:
            del checkpoint_ema['x_embedder.proj.weight']  # 删除x_embedder部分
    ...
    transform = True
    dataset = UKBDataset_modality_translation_DU15_zstat_finetune_uncon(args.data_path, args.data_file,  image_data_info ,transform=transform)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_info", type=str)# data_info
    parser.add_argument("--epochs", type=int, default=1650)
    ...
    parser.add_argument("--ckpt_path", type=str, help="Path to the checkpoint directory") # GenBrain pretraining ckpt dir
...
# torchrun --nnodes=1 --nproc_per_node=1 finetune_modality_translation_model_DU15_z1_uncon_step.py