...
def main(args):
    ...
    transform = True
    dataset = UKBDataset_modality_translation_T2_T1_finetune_uncon(args.data_path, args.data_file, args.label_file, transform=transform)
    ...
    for epoch in range(0, args.epochs):
        ...
        for data in loader:
            source_x = data['T2_image'].to(device)
            y1 = data['age'].to(device)
            y2 = data['sex'].to(device)
            y3 = data['mod'].to(device)
            target_x = data['T1_image'].to(device)
...
# torchrun --nnodes=1 --nproc_per_node=1 finetune_modality_translation_model_T2_T1_uncon_step.py