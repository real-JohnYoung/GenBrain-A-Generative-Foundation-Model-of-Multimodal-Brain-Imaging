...
class DiT_UKB_finetune(nn.Module):
    ...
    def __init__(self, dit_model, finetune_mode='full', finetune_block=None):
        ...
        self.dit_model.x_embedder = PatchEmbed_1d(input_size=self.dit_model.x_embedder.input_size, 
                                  ...
                                  in_chans=10, embed_dim=self.hidden_size)
...
def main(args):
    ...
    transform = True
    dataset = UKBDataset_modality_translation_DWI_DU15_finetune_uncon(args.data_path, args.data_file,  image_data_info ,transform=transform)
    ...
    for epoch in range(0, args.epochs):
        ...
        for data in loader:
            source_x = data['DWI_image'].to(device)
            y1 = data['age'].to(device)
            y2 = data['sex'].to(device)
            y3 = data['mod'].to(device)
            target_x = data['du_image'].to(device)
...
# torchrun --nnodes=1 --nproc_per_node=1 finetune_modality_translation_model_DWI_DU15_9_uncon_step.py