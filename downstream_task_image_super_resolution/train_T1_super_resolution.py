...
from model_utils import UKBDataset_T1_super_resolution,PatchEmbed_1d
...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = f"{args.results_dir}/{experiment_index:03d}-{model_string_name}-train-{args.finetune_mode}"  # Create an experiment folder
    ...
    image_part_num = args.image_part_num

    model = DiT_UKB_finetune(dit_model,image_part_num,finetune_mode,finetune_block)
    ...
    logger.info(f"Training DiT Parameters: {sum(p.numel() for p in model.parameters()):,}")

    opt = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-4, weight_decay=0)

    # Setup data (same as before):
    transform = True
    dataset = UKBDataset_T1_super_resolution(args.data_path, args.data_file, transform=transform)
    ...
    for epoch in range(0, args.epochs):
        ...
        for data in loader:
            ...
            model_kwargs = dict(y1=y1,y2=y2,y3=y3,y4=y4,lr_x=lr_x)
            ...
            # Save DiT checkpoint:
            if rank == 0 and (train_steps+1)%50000 ==0:
                checkpoint = {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str) # path to processed_1mm_1d MRI data root
    parser.add_argument("--data_file", type=str, default="PATH TO /T1_image_super_resolution_training_data_n_1k.csv")
    ...
    parser.add_argument("--global-batch-size", type=int, default=64)  # 512
...
# torchrun --nnodes=1 --nproc_per_node=2 train_T1_super_resolution.py
