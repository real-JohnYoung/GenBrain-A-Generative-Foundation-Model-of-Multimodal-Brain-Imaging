...
import argparse
import pandas as pd
from models import DiT_models,NumericalEmbedder
import json
import numpy as np
...
class DiT_UKB_finetune(nn.Module):
    """
    Diffusion model with nihss finetune module.
    """
    def __init__(self, dit_model, score_info, finetune_mode='full', finetune_block=None):
        super().__init__()
        
        self.dit_model = dit_model
        self.hidden_size = self.dit_model.y1_embedder.hidden_size
        self.score_embedder = NumericalEmbedder(score_info, hidden_size=self.hidden_size, dropout_prob=self.dit_model.class_dropout_prob)
        nn.init.zeros_(self.score_embedder.mlp[0].weight)
        nn.init.zeros_(self.score_embedder.mlp[2].weight)
    ...
    def set_finetune_mode(self):
        ...
        if self.finetune_mode == 'full':
            ...
            requires_grad(self.score_embedder, True)  # Unfreeze score embedder
        elif self.finetune_mode == 'new_module':
            requires_grad(self.dit_model, False)  # Freeze the base model
            requires_grad(self.score_embedder, True)  # Unfreeze only the score embedder
    ...
    def forward(self, x, t, y1, y2, y3, y4):
        """
        Forward pass of DiT with score embedder fine-tuning module.
        """
        ...
        y4 = self.score_embedder(y4, self.training, drop_mask)   # (N, D)      score
...
def main(args):
    ...
    score_info = {'mean':66.79,'std':27.38}
    finetune_mode = args.finetune_mode
    finetune_block = args.finetune_block

    model = DiT_UKB_finetune(dit_model,score_info,finetune_mode,finetune_block)
    ...
    id_labels = sample_data['participant_id'].astype(str).tolist()
    age_labels = sample_data['age_at_stroke'].astype(float).tolist()
    sex_labels = sample_data['sex'].map({'F': 0, 'M': 1}).astype(int).tolist()
    score_labels = sample_data['wab_aq'].astype(float).tolist()
    ...
    for i in range(0, n, batch_size):  # (0, n, batch_size)
        for sample_num in range(0,average_num):
            ...
            score_batch = score_labels[i:i + batch_size]

            real_sub_num = len(id_batch)
            mod_batch = [0] * real_sub_num
            ...
            y4 = torch.tensor(score_batch,  dtype=torch.float32, device=device)
            
            z = torch.cat([z, z], 0)  # Uncomment if intentionally doubling z

            real_batch_size = real_sub_num 

            y1_null = torch.tensor([0] * real_batch_size, dtype=torch.float32, device=device)
            ...
            y4_null = torch.tensor([0] * real_batch_size, dtype=torch.float32, device=device)
            ...
            save_batch_data(id_batch, mod_batch, samples.cpu(), save_dir, sample_num)

            print(f"Batch {i // batch_size + 1} saved successfully, sample num {sample_num}, saved successfully.")
        
        if args.get_average_samples ==1:
            get_average_samples(id_batch,mod_batch,average_num, save_dir)
...
def get_average_samples(eid_batch, modality_batch, average_num, save_dir):
    ...
    for i in range(sub_num):
        ...
        if modality == 0:
            modality_name = 'all_FA'
        modality_data = []
        for sample_num in range(average_num):
            data_path = os.path.join(save_dir,f"gen_{str(eid_batch[i])}",f"{eid_batch[i]}_{modality_name}_{sample_num}.npy")
            data = np.load(data_path)
            modality_data.append(data)
        modality_data = np.stack(modality_data)
        modality_data = np.mean(modality_data,axis=0)
        save_path = os.path.join(save_dir,f"gen_{str(eid_batch[i])}",f"{eid_batch[i]}_{modality_name}_average_{average_num}.npy")
        np.save(save_path, modality_data)

def save_batch_data(eid_batch, modality_batch, data, save_dir, sample_num):
    sub_num = len(eid_batch)
    for i in range(sub_num):
        addr = os.path.join(save_dir, f"gen_{str(eid_batch[i])}")
        os.makedirs(addr, exist_ok=True)
        x = data[i]
        modality = 'all_FA'
        x = (x * 0.152 + 0.162).numpy()
        np.save(os.path.join(addr, f"{eid_batch[i]}_{modality}_{sample_num}.npy"), x)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt", type=str, default=None) #checkpoints/step_0010000.pt
    ...
    parser.add_argument("--sample_file", type=str, default=None)# data aug file
    ...
    parser.add_argument("--get_average_samples", type=int, default=0) # need 1, not need 0  # clinical application sample one time 
