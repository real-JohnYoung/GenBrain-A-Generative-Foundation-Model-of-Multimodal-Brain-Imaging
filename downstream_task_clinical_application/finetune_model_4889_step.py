...
from models import DiT_models,NumericalEmbedder,LabelEmbedder
import sys
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from diffusion import create_diffusion
from model_utils import UKBDataset_clinical_4889_finetune
...
def main(args):
    ...
    if "ema" in checkpoint:
        ema.dit_model.load_state_dict(checkpoint["ema"])
        logger.info("Loaded EMA weights from checkpoint.")
    else:
        logger.warning("No EMA weights found in checkpoint.")

    print("Extend to new modalities...")
    model.dit_model.y3_embedder = ExtendedLabelEmbedder(model.dit_model.y3_embedder,n_new_modalities=1)               # ADC modality: 35
    ema.dit_model.y3_embedder = ExtendedLabelEmbedder(ema.dit_model.y3_embedder,n_new_modalities=1).to(device)        # ADC modality: 35
    ...
    transform = True
    dataset = UKBDataset_clinical_4889_finetune(args.data_path, args.data_label_file, image_data_info, transform=transform)
    ...
    for epoch in range(0, args.epochs):
        ...
        for data in loader:
            ...
            y4 = data['score'].to(device)

            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device)
            model_kwargs = dict(y1=y1,y2=y2,y3=y3,y4=y4)
            ...
            # Save DiT checkpoint:
            if rank == 0 and ((train_steps+1)%5000==0):
                checkpoint = {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str) # dwi_adc_1d data dir
    parser.add_argument("--data_label_file", type=str)# ds_4889_labels/training_dataset.csv
    ...
    parser.add_argument("--global-batch-size", type=int, default=32)  # 512
