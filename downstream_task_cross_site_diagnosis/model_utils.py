...
# This dataset is used for downstream task [liuyuan ad finetune].
class UKBDataset_liuyuan_ad_finetune(Dataset):
    ...
    def __getitem__(self, idx):
        """
        Finetune Label:
        - dis (torch.Tensor): The disease label as a tensor.  #[liuyuan ad]
        """
        # Get the row for the current index
        ...
        # Construct the .npy file path
        if modality == 11:      # T1 iamge
            image_path = os.path.join(self.data_path, npt_id ,"T1_brain_2mm_stdspace.npy")
        else:  # T2 image 
            image_path = os.path.join(self.data_path, npt_id, "T2_brain_2mm_stdspace.npy")
...
# This dataset is used for downstream task [liuyuan ad finetune for T1 modality].  liuyuan is also zic dataset
class liuyuan_ad_finetune_T1(Dataset):
    def __init__(self, data_path, data_label_file,image_data_info,transform=None):
        # Read the CSV file to get labels
        self.data_path = data_path
        self.data_label_file = pd.read_csv(data_label_file)
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image = torch.clamp(image, 0, 3000)
            mean = image.mean()
            std = image.std()
            image = (image-mean)/std
...
# This dataset is used for downstream task [adni ad finetune for T1 modality].
class adni_ad_finetune_T1(Dataset):
    def __init__(self, data_path, data_label_file,image_data_info,transform=None):
        # Read the CSV file to get labels
        self.data_path = data_path
        self.data_label_file = pd.read_csv(data_label_file)
    ...
    def __getitem__(self, idx):
        """
        Finetune Label:
        - dis (torch.Tensor): The disease label as a tensor.  #[adni ad]
        """
        ...
        sex = row['Sex'] 
        disease = row['label']
        image_path = os.path.join(self.data_path, file_name + ".npy")
...
# This dataset is used for downstream task [multisite scz finetune for T1 modality].
class UKBDataset_multisite_scz_finetune_T1(Dataset):
    def __init__(self, data_path, data_label_file,image_data_info,transform=None):
        """
            Based on UKBDataset.
        """
    ...
    def __getitem__(self, idx):
        """
        Finetune Label:
        - dis (torch.Tensor): The disease label as a tensor.  # [multisite scz]
        """
        # Get the row for the current index
        row = self.data_label_file.iloc[idx]
        sub_id= row['id']
        modality = 11    # only T1 data
