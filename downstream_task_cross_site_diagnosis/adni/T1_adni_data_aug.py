...
def main():
    ...
    train_file_path = 'Path_Toadni_nc_ad_mci/labels/adni_train_seg_data.csv'
    val_file_path = 'Path_Toadni_nc_ad_mci/labels/adni_val_seg_data.csv'
    test_file_path = 'Path_Toadni_nc_ad_mci/labels/adni_test_seg_data.csv'
    aug_file_path = 'Path_Toadni_nc_ad_mci/data_aug_labels/data_aug_data.csv'
    ...
    for i in range(0,1):
        ...
        train_data = np.array(train_file.iloc[:, 6:])

        mean = train_data.mean(axis = 0)
        std = train_data.std(axis = 0)
        train_data = (train_data-mean)/std

        test_data = np.array(test_file.iloc[:, 6:])
        test_data = (test_data-mean)/std

        val_data = np.array(val_file.iloc[:, 6:])
        ...
        output_root = f'Path_Toadni_nc_ad_mci/results_for_p_value/baseline_data_aug_{i*10}_percent_ft_{ft_step}_random'
