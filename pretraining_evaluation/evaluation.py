...
def main(args):
    ...
    for i in range(0, n, batch_size):  # (0, n, batch_size)
        ...
        save_batch_data(eid_batch, samples.cpu(), save_dir, image_data_info, args.modality_num)

        print(f"Batch {i // batch_size + 1} saved successfully.")
    
    compute_correlation(sample_data,save_dir)
...
def compute_correlation(gen_with_labels,save_dir):
    ...
    for modality in modalities:
        gen_data = []
        for eid in tqdm(gen_eids):
            data = np.load(os.path.join(save_dir,f"generated_{eid}",f'{modality}.npy'))
            gen_data.append(data)
        gen_data = np.array(gen_data).reshape(len(gen_data),-1)
        gen_age_coef = []
        gen_sex_coef = []
        for i in tqdm(range(228453)):
...
def save_batch_data(eid_batch, data, save_dir, image_data_info, modality_num):
