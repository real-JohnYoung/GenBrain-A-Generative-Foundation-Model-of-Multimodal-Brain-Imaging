...
def compute_similarity(bwas_dir, save_dir, index):
    ...
    for modality in modalities:
        ...
        r_gen_age = np.load(os.path.join(bwas_dir, f'test_{modality}_age.npy'))
        t_gen_age = r_gen_age * np.sqrt((n_test - 2) / (1 - r_gen_age ** 2))
        ...
        r_gen_sex = np.load(os.path.join(bwas_dir, f'test_{modality}_sex.npy'))
        t_gen_sex = r_gen_sex * np.sqrt((n_test - 2) / (1 - r_gen_sex ** 2))
...
save_dir = 'xxx/pretrain_bwas_analysis/pretrain_bwas_eavluation_1500_results/real'
real_root = 'xxx/compute_correlation_full'
for i in range(5):
    bwas_dir = os.path.join(gen_root,f'test_data_{i}')
