...
from model_utils import UKBDataset
...
def main(args):
    ...
    # Create model:
    model = DiT_models[args.model](condition_info=condition_info)
    ...
    dataset = UKBDataset(args.data_path, args.data_file,args.label_file,image_data_info,transform=transform)
    ...
    for epoch in range(args.epochs):
        ...
        for data in loader:
            ...
            y1 = data['age'].to(device).squeeze(1)
            y2 = data['sex'].to(device).squeeze(1)
            y3 = data['mod'].to(device)

            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device)
            model_kwargs = dict(y1=y1,y2=y2,y3=y3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_dir", type=str, default="SET_YOUR_MODEL_DIRECTORY/multimodal_foundation_model/")
    parser.add_argument("--model", type=str, choices=list(DiT_models.keys()), default="DiT-UKB-L/256")
    parser.add_argument("--global-batch-size", type=int, default=192) # 512
