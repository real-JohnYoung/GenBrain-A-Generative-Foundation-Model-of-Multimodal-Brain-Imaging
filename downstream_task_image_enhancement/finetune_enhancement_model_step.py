...
def main(args):
    ...
    if "ema" in checkpoint:
        ...
        if 'x_embedder.proj.weight' in checkpoint_ema:
            del checkpoint_ema['x_embedder.proj.weight']  # delate x_embedder part
    ...
    transform = True
    dataset = UKBDataset_enhancement_finetune(args.data_path, args.cor_data_path, args.data_file, image_data_info, transform=transform)
    ...
    for epoch in range(0, args.epochs):
        ...
        for data in loader:
            ...
            cor_x = data['cor_image'].to(device)

            t = torch.randint(0, diffusion.num_timesteps, (x.shape[0],), device=device)
            model_kwargs = dict(y1=y1,y2=y2,y3=y3,cor_x=cor_x)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--epochs", type=int, default=1650) # 50,000 step
    parser.add_argument("--results_dir", type=str, default="Path_to_Save_Directory/image_quality_enhancement")
    ...
    parser.add_argument("--ckpt_path", type=str, default='Path_to_Pretrain_ckpt_dir', help="Path to the checkpoint directory")
...
# torchrun --nnodes=1 --nproc_per_node=2 finetune_enhancement_model_step.py