...
def main(args):
    ...
    id_labels = sample_data['file_name'].astype(str).tolist()
    age_labels = sample_data['age'].astype(float).tolist()
    sex_labels = sample_data['Sex(M1F0)'].astype(int).tolist()
    dis_labels = sample_data['label'].astype(int).tolist()
    mod_labels = [15]*len(id_labels)
...
def save_batch_data(eid_batch,modality_batch, data, save_dir, image_data_info, sample_num):
    ...
    for i in range(sub_num):
        ...
        x = (x * 0.27 + 0.41).numpy()   # smoothed vbm modality data, mean 0.41, std 0.27
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sample_file", type=str, default=None)# other_site_data.csv
