...
from model_utils import UKBDataset_abide_autism_tian_subcortex_finetune_separately
...
def main(args):
    ...
    if rank == 0:
        ...
        experiment_dir = f"{args.results_dir}/{args.site_name}_sub_cortex_{args.subcortex_id}-{model_string_name}-finetune_mode-{args.finetune_mode}"  # Create an experiment folder
    ...
    model.dit_model.y3_embedder = ExtendedLabelEmbedder(model.dit_model.y3_embedder,n_new_modalities=1)            # tian subcortex : 35. add separately
    ema.dit_model.y3_embedder = ExtendedLabelEmbedder(ema.dit_model.y3_embedder,n_new_modalities=1).to(device)     # tian subcortex : 35  add separately
    ...
    transform = True
    dataset = UKBDataset_abide_autism_tian_subcortex_finetune_separately(args.data_path, args.data_label_file, image_data_info,subcortex_id=args.subcortex_id, transform=transform)
    ...
    for epoch in range(0, args.epochs):
        ...
        for data in loader:
            ...
            # Save DiT checkpoint:
            if rank == 0 and ((train_steps+1)%2000==0):
                checkpoint = {
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default=None)# other_site_data
    parser.add_argument("--data_label_file", type=str, default=None)
    parser.add_argument("--data_info", type=str, default=None)
    parser.add_argument("--epochs", type=int, default=2000)
    parser.add_argument("--results_dir", type=str, default=None)
    parser.add_argument("--site_name", type=str, default='NYU')
    ...
    parser.add_argument("--experiment_index", type=int, default=0, help="Experiment index ")
    ...
    parser.add_argument("--disease_num", type=int, default=2) # NC Autism

    args = parser.parse_args()
    main(args)


# torchrun --nnodes=1 --nproc_per_node=1 finetune_abide_autism_one_site_tian_subcortex_step_separate.py